...
import CheckBlocPage from "../components/CheckBlocPage";
...
const Page = () => {
  ...
  useEffect(() => {
    ...
    setFiles([]);
    setOthers([]);
      allIntegrations.length>0 && allIntegrations?.forEach((item: any) => {
    ...
    <main className="w-full h-full flex">
      ...
      <div className="w-full h-full flex flex-col items-center justify-start ml-20 md:ml-24">
        ...
        <div className="flex flex-col items-center justify-center gap-9 w-full mb-20">
          ...
          <ProgressBar c1="bg-[#0784C6]" c2="bg-[#0784C6]" />
          <CheckBlocPage website={websites} file={files} other={others} />
          <div className="flex items-center justify-between w-1/2">
            <button onClick={()=>router.push('/new/creates/add')} className="bg-[#292929] rounded px-6 py-2 items-center w-48 font-spacegrotesk font-medium text-sm text-[#FFFFFFCC]">
              previous
            </button>
            <button
              onClick={openModal}
              className="bg-[#0784C6] rounded px-6 py-2 items-center w-48 font-spacegrotesk font-medium text-sm text-[#FFFFFFCC]"
            >
              publish bloc
            </button>
          </div>
      ...
      <Modal
        ...
        >
        <ShareModal name={blocData.name} url={url} BlocId={token} homePage={false}  />
        {/* <div className="flex flex-col w-[35vw] h-[520px] p-8 py-10 gap-5 items-center justify-start bg-[#181818] text-white rounded-lg">
          ...
          >
            ...
            >
              <div className="text-center font-spacegrotesk text-2xl font-medium text-[#FFFFFFCC]">Embed on Web App</div>
            ...
            >
              <div className="text-center font-spacegrotesk text-2xl font-medium text-[#FFFFFFCC]">Open on Mobile</div>
              <div className="text-center font-spacegrotesk font-medium text-sm leading-[18px] text-[#FCFCFD]">
          ...
          <div className="flex flex-col items-center justify-center gap-4 w-full">
            <button onClick={()=>router.push(`/new/bloc/${blocId}/settings`)} className="bg-[#0784C6] text-sm font-spacegrotesk font-medium rounded px-9 py-3">
              manage bloc
            </button>
            <button onClick={()=>router.push('/new/dashboard')} className="bg-transparent text-sm font-spacegrotesk font-medium">
              go to dashboard
            </button>
          </div>
        </div> */}
